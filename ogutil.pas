  ...
  if midDrives in MachineInfo then begin
    ...
    for Drive := 'C' to 'Z' do begin
      ...
      if (GetDriveType(PChar(Drive + ':\')) = DRIVE_FIXED) then begin
        ...
        *)
        OutputDebugString(PChar(Format('CreateMachineID:midDrives %s:\ %s', [Drive, {$IFDEF DELPHI17UP}System.AnsiStrings.StrPas(subst){$ELSE}StrPas(subst){$ENDIF}])));
        // modified end
        ...
        *)
        if(Copy({$IFDEF DELPHI17UP}System.AnsiStrings.StrPas(subst){$ELSE}StrPas(subst){$ENDIF}, 1, 4)) <> '\??\' then begin                        {!!.15}
        // modified end
...
begin
  ...
  for I := 1 to Length(Hex) do
    ...
    *)
    if(IsDigit(Hex[I])) or (CharInSet(Hex[I], ['0'..'9', 'A'..'F', 'a'..'f'])) then Str := Str + Hex[I];
    // modified end
    {$ELSE}
